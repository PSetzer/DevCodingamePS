...
class Game
{
                        ...
                        if (card != null)
                        {
                            ...
                            else if (card.type == 1)
                            {
                                ...
                                if (myCreatureTarget != null)
                                {
                                    ...
                                    // gérer ajout abilities en bitwise
                                }
                            }
                            else if (card.type > 1)
                            {
                                ...
                                if (oppCreatureTarget != null)
                                {
                                    ...
                                    meMC.health += card.myHealthChange;
                                    oppMC.health += card.opponentHealthChange;
                                    // gérer suppression abilities en bitwise
                                }
                                else if (card.type == 3)
                                {
    ...
    static void AttackPhase(Player meMC, Player oppMC, List<string> listActions)
    {
        while (meMC.lstCardsOnBoard.Any(x => !x.hasAttacked && !(x.wasJustSummoned && !x.hasCharge) && x.attack > 0))
        {
            ...
            if (defender != null)
            {
                ...
                if (attacking != null)
                {
                    AttackOpponentCard(attacking, defender, listActions);
                }
            }
            else
            {
    ...
    static double ScoreMC(Player meMC, Player oppMC)
    {
        ...
        if (meMC.health == 0) score = -1000;
        else if (oppMC.health == 0) score = 1000;
        else
        {
    ...
    static void AttackOpponentCard(Card myCreature, Card opponentCreature, List<string> listActions)
    {
...
class Card : ICloneable
{
    ...
    public void ReinitWith(Card init)
    {
        ...
        hasBreakthrough = init.hasBreakthrough;
        hasCharge = init.hasCharge;
        hasDrain = init.hasDrain;
        isGuard = init.isGuard;
        isLethal = init.isLethal;
        hasWard = init.hasWard;
