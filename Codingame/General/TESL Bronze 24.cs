...
class Game
{
        ...
        while (true)
        {
            ...
            else
            {
                ...
                while (me.lstCardsOnBoard.Any(x => !x.hasAttacked && !(x.wasJustSummoned && !x.hasCharge) && x.attack > 0))
                {
                    ...
                    if (defender == null)
                        defender = opponent.lstCardsOnBoard.FirstOrDefault(x => x.isGuard && x.defense == maxOppDefense && readyLethal != null);
                    if (defender == null)
                        defender = opponent.lstCardsOnBoard.FirstOrDefault(x => x.isGuard);
                    if (defender == null)
                    ...
                    if (defender != null)
                    {
                        ...
                        if (attacking != null)
                        {
                            AttackOpponentCard(attacking, defender, me, opponent);
                        }
                    }
                    else
                    {
    ...
    static void AttackOpponentCard(Card myCreature, Card opponentCreature, Player me, Player opponent)
    {
