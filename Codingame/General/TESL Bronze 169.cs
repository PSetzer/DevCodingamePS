...
class Game
{
        ...
        while (true)
        {
            ...
            else
            {
                ...
                while (summoned)
                {
                    ...
                    toUseOnMyCreature = me.lstCardsInHand.FirstOrDefault(x => x.type == 1 && x.cost <= me.mana);
                    ...
                    toUseOnOppCreature = me.lstCardsInHand.FirstOrDefault(x => x.type > 1 && x.cost <= me.mana);
                ...
                // phase d'attaque
                while (me.lstCardsOnBoard.Any(x => !x.hasAttacked && !(x.wasJustSummoned && !x.hasCharge)))
                {
                    List<Card> lstAttackingCards = me.lstCardsOnBoard.Where(x => !x.hasAttacked && !(x.wasJustSummoned && !x.hasCharge)).ToList();
    ...
    static void PickCard()
    {
        ...
        {
            objectRate = lstCardInDeck.Count(x => x.type > 0) / lstCardInDeck.Count;
            lowCostRate = lstCardInDeck.Count(x => x.cost < 4) / lstCardInDeck.Count;
        }

        PrintLine(objectRate);
        ...
        for (int i = 0; i < me.lstCardsInHand.Count; i++)
        {
            ...
            if (lstCardInDeck.Count > 10 && lowCostRate < 0.50 && cardInHand.cost > 3)
                continue;
...
class Card
{
    ...
    public override string ToString()
    {
        return $"id : {id} loc : {location} cost : {cost} a : {attack} d : {defense} ab : {abilities}\n";
    }
}
